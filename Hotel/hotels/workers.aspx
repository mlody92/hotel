<%@ Page Title="" Language="C#" MasterPageFile="~/Site2.Master" AutoEventWireup="true" CodeBehind="workers.aspx.cs" Inherits="Hotel.hotels.workers" %>
                        ...
                        <asp:GridView GridLines="None" CellSpacing="-1" class="bordered" ID="allhoteleGridView" runat="server" DataSourceID="allhoteleSqlDataSource" AutoGenerateColumns="False" DataKeyNames="id" OnRowCommand="OnRowCommand">
                            <Columns>
                                ...
                                <asp:TemplateField ShowHeader="False" ItemStyle-Width="140px">
                                    <ItemTemplate>
                                        <asp:LinkButton ID="LinkButton1" runat="server" CommandName="wybierz" Text="Wybierz" CommandArgument='<%#Eval("dzialka_user_id")%>' />
                                    </ItemTemplate>
                                </asp:TemplateField>
                            </Columns>
                        </asp:GridView>
                        <asp:SqlDataSource ID="allhoteleSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" SelectCommand="SELECT dzialka.id, dzialka.id_miasto, dzialka.nazwa, dzialka.cena, miasto.nazwa AS nazwaMiasto, wojewodztwo.nazwa AS nazwaWoj, rodzaj_hotelu.rodzaj, atrakcyjnosc.nazwa AS nazwaAtr, dzialka_user.id AS dzialka_user_id FROM dzialka INNER JOIN miasto ON dzialka.id_miasto = miasto.id INNER JOIN rodzaj_hotelu ON dzialka.rodzaj_hotelu = rodzaj_hotelu.id INNER JOIN atrakcyjnosc ON dzialka.rodzaj_hotelu = atrakcyjnosc.id INNER JOIN wojewodztwo ON miasto.id_wojewodztwo = wojewodztwo.id INNER JOIN dzialka_user ON dzialka.id = dzialka_user.dzialka_id WHERE (dzialka.id IN (SELECT dzialka_id FROM dzialka_user AS dzialka_user_1 WHERE (uzytkownik_id = @uzytkownik_id))) AND (dzialka.status LIKE 'aktywny')"></asp:SqlDataSource>
                    </p>
                </div>

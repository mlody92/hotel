<%@ Page Title="" Language="C#" MasterPageFile="~/Site2.Master" AutoEventWireup="true" CodeBehind="manage.aspx.cs" Inherits="Hotel.hotels.manage" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="MiddleContentPlaceHolder" runat="server">
    <div class="post">
        <div class="postheader">
            <h1>Kup działkę</h1>
            <p><asp:Label ID="komunikatLabel" runat="server" Text="Label" ForeColor="Red" Visible="false"></asp:Label></p>
        </div>
            ...
            <div id="body">
                <div>
                    <p>
                        ...
                        <asp:LinkButton ID="filtrujButton" runat="server" OnClick="filtrujButton_Click">Filtruj</asp:LinkButton>
                ...
                <div>
                    <p>
                        <asp:GridView GridLines="None" CellSpacing="-1" class="bordered" ID="filterHoteleGridView" runat="server" DataSourceID="filterSqlDataSource" AutoGenerateColumns="False" DataKeyNames="id" OnRowCommand="OnRowCommand">
                            <Columns>
                                ...
                                <asp:TemplateField ShowHeader="False" ItemStyle-Width="140px">
                                    <ItemTemplate>
                                        <asp:LinkButton ID="LinkButton1" runat="server" CommandName="kup" Text="Kup" CommandArgument='<%#Eval("id")%>' />
                                    </ItemTemplate>
                                </asp:TemplateField>
                            </Columns>
                        </asp:GridView>
                        <asp:SqlDataSource ID="filterSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" SelectCommand="SELECT dzialka.id,dzialka.id_miasto,dzialka.nazwa, dzialka.cena, miasto.nazwa AS nazwaMiasto, wojewodztwo.nazwa AS nazwaWoj, rodzaj_hotelu.rodzaj, atrakcyjnosc.nazwa AS nazwaAtr FROM dzialka INNER JOIN miasto ON dzialka.id_miasto = miasto.id INNER JOIN rodzaj_hotelu ON dzialka.rodzaj_hotelu = rodzaj_hotelu.id INNER JOIN atrakcyjnosc ON dzialka.rodzaj_hotelu = atrakcyjnosc.id INNER JOIN wojewodztwo ON miasto.id_wojewodztwo = wojewodztwo.id WHERE ([id_miasto]=@id_miasto) AND (dzialka.id NOT IN (Select dzialka_id FROM dzialka_user WHERE [uzytkownik_id]=@uzytkownik_id)) AND dzialka.status LIKE 'aktywny'">
                            <SelectParameters>
                        ...
                        <asp:SqlDataSource ID="allhoteleSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" SelectCommand="SELECT        dzialka.id, dzialka.id_miasto, dzialka.nazwa, dzialka.cena, miasto.nazwa AS nazwaMiasto, wojewodztwo.nazwa AS nazwaWoj, rodzaj_hotelu.rodzaj, atrakcyjnosc.nazwa AS nazwaAtr FROM dzialka INNER JOIN miasto ON dzialka.id_miasto = miasto.id INNER JOIN rodzaj_hotelu ON dzialka.rodzaj_hotelu = rodzaj_hotelu.id INNER JOIN atrakcyjnosc ON dzialka.rodzaj_hotelu = atrakcyjnosc.id INNER JOIN wojewodztwo ON miasto.id_wojewodztwo = wojewodztwo.id WHERE (dzialka.id NOT IN (Select dzialka_id FROM dzialka_user WHERE [uzytkownik_id]=@uzytkownik_id)) AND dzialka.status LIKE 'aktywny'"></asp:SqlDataSource>
                    </p>
                </div>

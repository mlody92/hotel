...
{
    public partial class editWojewodztwo : System.Web.UI.Page
    {
        Hotel.Admin.tools tools = new Hotel.Admin.tools();
        String[] dane = new String[2];
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["Admin_id_woj"] != null)
            {
                if (getWojewodztwo(Session["Admin_id_woj"].ToString()) == true)
                {
                ...
                else
                {
                    ...
                    komunikatLabel.Text = "Nie pobrano danych wojewodztwa.";
                }
            }
            else Server.Transfer("~/Admin/region/manageWoj.aspx");
        }

        protected void zapiszButton_Click(object sender, EventArgs e)
        {
            checkBox();
            tools.updateWojewodztwo(dane[0], nazwaTextBox.Text);
            komunikatLabel.Visible = true;
        ...
        {
            Session["Admin_id_woj"] = null;
            Server.Transfer("~/Admin/region/manageWoj.aspx");
        }
